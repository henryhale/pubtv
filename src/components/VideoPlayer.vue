            ...
            <div class="flex-centered space-x-2 mr-2">
                <button @click="() => toggleMute(state, video, volume)">
                    <svg>
                        ...
                        <use v-else-if="!state.muted && state.volume > 0 && state.volume <= 0.5" href="#low-volume">
                        </use>
                        <use v-else href="#high-volume"></use>
                    </svg>
                </button>
                <input ref="volume" v-model="state.volume" @change="() => changeVolume(state, video, volume)"
                    data-volume="0.5" type="range" min="0" max="1" step="0.01">
            </div>
            <div class="flex-centered">
...
video {
    @apply w-full h-full max-w-full max-h-full border-2 border-black;
